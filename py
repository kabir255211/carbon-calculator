"""
TREE CARBON CONSUMPTION CALCULATOR
==================================

LIBRARIES USED:
---------------
1. streamlit      - Web framework for creating interactive Python web apps (handles UI, inputs, buttons)
2. streamlit_lottie - Displays animated Lottie JSON files in Streamlit (the floating tree)
3. requests       - HTTP library to fetch Lottie animation from URL
4. pandas         - Data manipulation library (displays results in table format)

RUN: streamlit run tree_carbon_calculator.py
"""
...
# Tree CO2 absorption rates (kg/year)
INDIAN_TREES = {
    "Peepal": 22.0,
    "Neem": 18.0,
    "Banyan": 25.0,
...
if st.button("Calculate Carbon Absorption", use_container_width=True):
    ...
    total_co2_kg = num_trees * co2_rate * years
...
# Tree rates reference
with st.expander("View All Tree CO2 Rates"):
    st.dataframe(pd.DataFrame({"Tree": INDIAN_TREES.keys(), "CO2 (kg/year)": INDIAN_TREES.values()}))
